    ...
    for root, dirs, files in chain.from_iterable(os.walk(path) for path in paths):
        ...
        for file in files:
            ...
            contents, alias_load_tag_count = re.subn(
                r"{% load cms_tags ",
                r"{% load cms_tags djangocms_alias_tags ",
                contents,
            )
...
def _remap_static_placeholder_plugins_to_static_alias(
    static_placeholder_id,
    static_placeholder_code,
    alias,
    migration_user,
...
def _process_static_placeholders():
    ...
    for static_placeholder in StaticPlaceholder.objects.all():
        ...
        # Get or create Alias
        alias = _get_or_create_alias(
            alias_category, static_placeholder.code, static_placeholder.site
        )
